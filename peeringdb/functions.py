...
def get_shared_internet_exchanges(as1: Network, as2: Network) -> QuerySet[IXLan]:
    ...
    return IXLan.objects.filter(pk__in=ixlan_ids_1) & IXLan.objects.filter(pk__in=ixlan_ids_2)


def get_possible_peering_sessions(as1: Network, as2: Network, ixlan: IXLan | None = None) -> QuerySet[NetworkIXLan]:
    # If both AS are the same or one of each has no PeeringDB record
    ...
    if ixlan:
        ixlan_ids = NetworkIXLan.objects.filter(net=as1, ixlan=ixlan).values_list("ixlan", flat=True)
    else:
...
def get_shared_facilities(as1: Network, as2: Network) -> QuerySet[Facility]:
    ...
    return Facility.objects.filter(pk__in=netfac_ids_1) & Facility.objects.filter(pk__in=netfac_ids_2)
