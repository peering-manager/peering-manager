...
class PeeringRequestViewSet(PeeringManagerModelViewSet):
    ...
    def accept(self, request, pk=None):
        if not request.user.has_perm("peering.review_peering_request"):
            return Response(status=status.HTTP_403_FORBIDDEN)
        peering_request = self.get_object()
        try:
            result = peering_request.accept()
    ...
    @extend_schema(
        operation_id="peering_peering_requests_reject",
        responses={
            200: OpenApiResponse(description="Request rejected"),
            400: OpenApiResponse(description="Cannot reject (wrong status)"),
        },
    )
    @action(detail=True, methods=["post"])
    def reject(self, request, pk=None):
        ...
        try:
            peering_request.reject(comment=comment)
        except ValueError as e:
            return Response({"detail": str(e)}, status=status.HTTP_400_BAD_REQUEST)
...
class RequestedSessionViewSet(PeeringManagerModelViewSet):
    ...
    @extend_schema(
        operation_id="peering_requested_sessions_reject",
        responses={
            200: OpenApiResponse(description="Session rejected"),
            400: OpenApiResponse(description="Cannot reject (wrong status)"),
        },
    )
    @action(detail=True, methods=["post"])
