...
# Nokia SROS Configuration templates
...
## Nokia SROS Sub template Filters
```
filter {
...
```
{# By default, every IX will be a BGP group with an Apply Group from generic config and default common policy settings #}
...
{% for ixp in internet_exchange_points %}
  {% for family in (6, 4) %}
    ...
    group {{ p }}IXP-{{ ixp.name }}-V{{ family }} {
      ...
      import {
        policy [
          "{{ p }}{{ ixp | iter_import_policies(family=family) | reverse | join('" "'+p) }}"
          {% if gshut %} "PEER-IN-GSHUT" {% endif %}
          {% if reject %} "BGP-REJECT-IN" {% else %} "BGP-ACCEPT-IN" {% endif %}
        ]
      }
      {% endif %}
      ...
      export {
        policy [
          "{{ p }}{{ ixp | iter_export_policies(family=family) | reverse | join('" "'+p) }}"
          {% if gshut %} "PEER-OUT-GSHUT" {% endif %}
          {% if reject %} "BGP-REJECT-OUT" {% else %} "BGP-ACCEPT-OUT" {% endif %}
        ]
      }
      {% endif %}
    }
...
{% for group in bgp_groups %}
  {% for family in (6, 4) %}
    ...
    group {{ p }}{{ group.name }}-V{{ family }} {
      ...
      import {
        policy [
          "{{ p }}{{ group | iter_import_policies(family=family) | reverse | join('" "'+p) }}"
          {% if gshut %} "PEER-IN-GSHUT" {% endif %}
          {% if reject %} "BGP-REJECT-IN" {% else %} "BGP-ACCEPT-IN" {% endif %}
        ]
      }
      ...
      export {
        policy [
          "{{ p }}{{ group | iter_export_policies(family=family) | reverse | join('" "'+p) }}"
          {% if gshut %} "PEER-OUT-GSHUT" {% endif %}
          {% if reject %} "BGP-REJECT-OUT" {% else %} "BGP-ACCEPT-OUT" {% endif %}
        ]
      }
...
```
{# Loop through Internet Exchange Points (IXPs) #}
{% for ixp in internet_exchange_points %}
  {% for family in (6, 4) %}
    ...
    {% for session in router | ixp_sessions(family=family, ixp=ixp) %}
      ...
      {% if session.enabled or disabled or 'maintenance' in session.status %}
        neighbor {{ session | ip }} {
          ...
          {# BGP Timers if set #}
          {% if session.config_context and session.config_context.timers.hold %}
          hold-time {
          ...
          import {
            policy [
              "{{ p }}{{ session | merge_import_policies('reverse') | iterate('name') | join('" "'+p) }}"
              {% if session | communities | length or session.autonomous_system | communities | length %}
              "{{ p }}COMM-{{ ixp.name }}-AS{{ session.autonomous_system.asn }}-V{{ family }}-IN"
              {% endif %}
              "{{ p }}FROM-AS{{ session.autonomous_system.asn }}-V{{ family }}"
              {% if gshut %} "PEER-IN-GSHUT" {% endif %}
              {% if reject %} "BGP-REJECT-IN" {% else %} "BGP-ACCEPT-IN" {% endif %}
            ]
          }
          ...
          export {
            policy [
              "{{ p }}{{ session | merge_export_policies('reverse') | iterate('name') | join('" "'+p) }}"
              {% if gshut %} "PEER-OUT-GSHUT" {% endif %}
              {% if reject %} "BGP-REJECT-OUT" {% else %} "BGP-ACCEPT-OUT" {% endif %}
            ]
          }
...
{# Loop through BGP Groups for Direct Neighbors #}
{% for group in bgp_groups %}
  {% for family in (6, 4) %}
        ...
        neighbor {{ session | ip }} {
          ...
          import {
            policy [
              "{{ p }}{{ session | merge_import_policies('reverse') | iterate('name') | join('" "'+p) }}"
              {% if session | communities | length or session.autonomous_system | communities | length %}
              "{{ p }}COMM-{{ group.name }}-AS{{ session.autonomous_system.asn }}-V{{ family }}-IN"
              {% endif %}
              "{{ p }}FROM-AS{{ session.autonomous_system.asn }}-V{{ family }}"
              {% if gshut %} "PEER-IN-GSHUT" {% endif %}
              {% if reject %} "BGP-REJECT-IN" {% else %} "BGP-ACCEPT-IN" {% endif %}
...
{%- for policy in routing_policies %}
  {%- set dont_export = policy | has_tag('dont-export') %}
  
  {# If the router-policy tag is set, use the Router context vars for community-based settings #}
...
{# Communities Policy Statement Parts #}
        ...
        policy-statement "{{ p }}COMM-{{ ixp.name }}-AS{{ session.autonomous_system.asn }}-V{{ family }}-IN" {
          entry 10 {
            action {
              ...
              community {
                add [
                  {%- if session | communities | length %}
                    "{{ p }}{{ session | communities | join('" "'+p) }}"
                  {%- endif %}
                  {%- if session.autonomous_system | communities | length %}
                    "{{ p }}{{ session.autonomous_system | communities | join('" "'+p) }}"
                  {%- endif %}
                ]
              }
            }
  ...
  {%- for tag in as.tags.all() if tag.slug == "filter-prefixes-v4" %}
    ...
    prefix-list "{{ p }}FROM-AS{{ as.asn }}-V4" {
      {% if as.prefixes.ipv4 %}
        ...
        prefix {{ thisprefix.prefix }} 
        {%- if not thisprefix.exact %}  
        type range {
