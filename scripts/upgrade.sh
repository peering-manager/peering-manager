...
${PYTHON} -c 'import sys; exit(1 if sys.version_info < (3, 12) else 0)' || {
  echo "--------------------------------------------------------------------"
  echo "ERROR: Unsupported Python version: ${PYTHON_VERSION}. Peering"
  echo "Manager requires Python 3.12 or later. To specify an alternate Python"
  echo "executable, set the PYTHON environment variable. For example:"
  echo ""
  echo "  PYTHON=/usr/bin/python3.12 ./upgrade.sh"
  echo ""
